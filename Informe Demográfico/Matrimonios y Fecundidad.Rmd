...
```{r}
matrimonios_edad_22_graf <- pivot_longer(matrimonios_edad_22, cols = c(`Hombres total`,`Hombres con mujeres`,`Hombres con hombres`,`Mujeres total`, `Mujeres con hombres`, `Mujeres con mujeres`), names_to = "Composición", values_to = "Cantidad")
```

```{r}
ggplot(matrimonios_edad_22_graf, aes(x = `Grupos de edades`, y = Cantidad, fill = Composición)) + 
  geom_line() + 
  geom_point() + 
  scale_fill_manual(values = c(`Hombres total` = "#2F4F4F", `Hombres con mujeres` = "cadetblue3", `Hombres con hombres` = "#BCEE68",`Mujeres total` = "#6E8B3D", `Mujeres con hombres` = "#AEEEEE", `Mujeres con mujeres` = "#B4EEB4"), name = "Tipo de Composición") +
  labs(title = "Costa Rica. Total de matrimonios por composición de la pareja.", subtitle = "2022-2023") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
        plot.caption = element_text(hjust = 0, color = "black", size = 8)) +
